...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	if (g_menu.main.aimbot.head1.get(2) && !(record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK))
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });
...
void Aimbot::find() {
	...
	if (best.player && best.record) {
		...
		if (g_cl.m_weapon_id == GLOCK || g_cl.m_weapon_id == P2000 || g_cl.m_weapon_id == USPS || g_cl.m_weapon_id == ELITE || g_cl.m_weapon_id == P250 || g_cl.m_weapon_id == TEC9 || g_cl.m_weapon_id == CZ75A || g_cl.m_weapon_id == DEAGLE || g_cl.m_weapon_id == REVOLVER)
			on = g_menu.main.aimbot.hitchance_pistols.get() && g_menu.main.misc.mode.get() == 0;
		else if (g_cl.m_weapon_id == SSG08)
			on = g_menu.main.aimbot.hitchance_scout.get() && g_menu.main.misc.mode.get() == 0;
		else if (g_cl.m_weapon_id == G3SG1 || g_cl.m_weapon_id == SCAR20)
			on = g_menu.main.aimbot.hitchance_auto.get() && g_menu.main.misc.mode.get() == 0;
		else if (g_cl.m_weapon_id == AWP)
			on = g_menu.main.aimbot.hitchance_awp.get() && g_menu.main.misc.mode.get() == 0;
		else
			on = g_menu.main.aimbot.hitchance_general.get() && g_menu.main.misc.mode.get() == 0;
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		if (g_aimbot.m_damage_toggle) {
			...
			if (g_cl.m_weapon_id == GLOCK || g_cl.m_weapon_id == P2000 || g_cl.m_weapon_id == USPS || g_cl.m_weapon_id == ELITE || g_cl.m_weapon_id == P250 || g_cl.m_weapon_id == TEC9 || g_cl.m_weapon_id == CZ75A || g_cl.m_weapon_id == DEAGLE || g_cl.m_weapon_id == REVOLVER) {
				...
				if (g_menu.main.aimbot.minimal_damage_hp_scout.get())
					dmg = std::ceil((dmg / 100.f) * hp);

				pendmg = g_menu.main.aimbot.penetrate_minimal_damage_scout.get();
				if (g_menu.main.aimbot.penetrate_minimal_damage_hp_scout.get())
					pendmg = std::ceil((pendmg / 100.f) * hp);
				...
				if (g_menu.main.aimbot.minimal_damage_hp_awp.get())
					dmg = std::ceil((dmg / 100.f) * hp);

				pendmg = g_menu.main.aimbot.penetrate_minimal_damage_awp.get();
				if (g_menu.main.aimbot.penetrate_minimal_damage_hp_awp.get())
					pendmg = std::ceil((pendmg / 100.f) * hp);
...
void Aimbot::apply() {
	...
	// red impacts
	if (g_menu.main.misc.forceimpact.get()) {
