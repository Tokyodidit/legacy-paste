...
void Checkbox::draw() {
	...
	render::rect(p.x - 2, p.y - 2, CHECKBOX_SIZE, CHECKBOX_SIZE, { 50, 50, 57, m_parent->m_alpha });
	...
	// render border.
	render::rect(p.x, p.y, CHECKBOX_SIZE - 4, CHECKBOX_SIZE - 4, { 50, 50, 57, m_parent->m_alpha });

	// render checked.
	if (m_checked) {
		render::rect_filled(p.x - 1, p.y - 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, color);
		render::rect_filled_fade(p.x - 1, p.y - 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, { 30, 30, 37, m_parent->m_alpha }, 0, 150);
	}

	else
		render::gradient(p.x - 1, p.y - 1, CHECKBOX_SIZE - 2, CHECKBOX_SIZE - 2, { 35, 35, 42, m_parent->m_alpha }, { 30, 30, 37, m_parent->m_alpha });
