...
void HVH::FakeFlick() {
	...
	auto* LocalCmd = g_cl.m_local;
	float server_time = LocalCmd->m_nTickBase() * g_csgo.m_globals->m_interval;
	if (server_time >= LastLBYUpdateTime) {
		LastLBYUpdateTime = server_time + 0.75f;
		wilupdate = true;
		g_cl.m_cmd->m_view_angles.y -= 173.f;
	}
	else {
		wilupdate = false;
		g_cl.m_cmd->m_view_angles.y += 173.f;
	...
	if (server_time >= LastLBYUpdateTime) {
		...
		wilupdate = true;
		g_cl.m_cmd->m_view_angles.y += 173;
	}
	else {
		wilupdate = false;
		g_cl.m_cmd->m_view_angles.y += 173.f;
	...
	if (server_time >= LastLBYUpdateTime) {
		...
		wilupdate = true;
		g_cl.m_cmd->m_view_angles.x += 89;
	}
	else {
		wilupdate = false;
		g_cl.m_cmd->m_view_angles.x += -180;
	}
}
			...
			// there will be an lbyt update on this tick.
				if (stand) {
					...
					case 1:
						...
						// random.
					case 2:
						g_cl.m_cmd->m_view_angles.y += g_csgo.RandomInt(-120.f, 120.f);
					...
					case 3:
						...
						// autistic.
					case 4:
						negative ? g_cl.m_cmd->m_view_angles.y += 84.f : g_cl.m_cmd->m_view_angles.y -= 64.f;
						negative = !negative;
						//if (g_menu.main.antiaim.pitch_fake_stand.get())
							//	g_cl.m_cmd->m_view_angles.x = -89.f;
						break;
					...
					case 3:
						...
						// random.
					case 4:
						g_cl.m_cmd->m_view_angles.y += g_csgo.RandomInt(-180.f, 180.f);
						break;
					}
				}
		}
...
void HVH::DoFakeAntiAim() {
	...
	*g_cl.m_packet = true;
	...
	case 2:
		...
		g_cl.m_cmd->m_view_angles.y += g_csgo.RandomFloat(-15.f, 25.f);
		break;
	...
	case 7: {
		...
		g_cl.m_cmd->m_view_angles.y = m_direction - 180.f;
		if (switcher)
			g_cl.m_cmd->m_view_angles.y += 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 90.f);
		else
			g_cl.m_cmd->m_view_angles.y -= 90 + std::fmod(g_csgo.m_globals->m_curtime * -105.f, 65.f);
		switcher = !switcher;
		break; }
...
void HVH::SendPacket() {
	...
	if (g_aimbot.CanDT() && g_menu.main.aimbot.akuzytap.get()) {
