...
void Keybind::draw() {
	...
	if (!m_set && !(m_key >= 0 && m_key <= 0xFE))
		render::esp_small.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 122, p.y + 15 + 4 - 19, color, XOR("[NO]"));

	if (!m_set && m_key >= 0 && m_key <= 0xFE)
		render::esp_small.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 122, p.y + 15 + 4 - 19, color, XOR(""));

	// todo; animate with '...'
	if (m_set)
		render::esp_small.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 122, p.y + 15 + 4 - 19, color, XOR("[KEY]"));

	// we have a key assigned.
	else if (m_key >= 0 && m_key <= 0xFE)
		render::esp_small.string(p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 122, p.y + 15 + 4 - 19, { color.r(),  color.g(),  color.b(), m_parent->m_alpha }, keynames[m_key]);
}
...
void Keybind::click() {
	...
	Rect bind = { p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET + 122, p.y + 15 + 4 - 19, m_w - KEYBIND_X_OFFSET + 10, KEYBIND_BOX_HEIGHT };
