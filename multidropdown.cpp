...
void MultiDropdown::draw() {
	...
	// draw label.
	if (m_use_label)
		render::menu_b.string(p.x + LABEL_OFFSET, p.y - 2, { 205, 205, 205, m_parent->m_alpha }, m_label);

	// draw border.
	render::rect(p.x + DROPDOWN_X_OFFSET, p.y + m_offset, m_w - DROPDOWN_X_OFFSET, DROPDOWN_BOX_HEIGHT, { 0, 0, 0, m_parent->m_alpha });

	// draw inside.
	render::rect_filled(p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + 1, m_w - DROPDOWN_X_OFFSET - 2, DROPDOWN_BOX_HEIGHT - 2, { 35, 35, 35, m_parent->m_alpha });

	// pand bar
	//render::rect_filled(p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + 1, 2, DROPDOWN_BOX_HEIGHT - 2, color);

	// arrow.
	//render::rect_filled(p.x + m_w - 1 - (DROPDOWN_BOX_HEIGHT / 2) - 3 + 1, p.y + m_offset + (DROPDOWN_BOX_HEIGHT / 4), (DROPDOWN_BOX_HEIGHT - 2) / 2, (DROPDOWN_BOX_HEIGHT - 2) / 2, color);
	...
	if (!m_items.empty()) {
		...
		if (m_open) {
			...
			// draw items inside.
			render::rect_filled(p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_SEPARATOR + 1, m_w - DROPDOWN_X_OFFSET - 2, m_anim_height - 1, { 35, 35, 35, m_parent->m_alpha });
			...
			for (size_t i{}; i < m_items.size(); ++i) {
				...
				if (m_anim_height > item_offset) {
					...
					render::menu_b.string(p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_ITEM_Y_OFFSET + item_offset,
						active ? color : Color{ 152, 152, 152, m_parent->m_alpha },
						m_items[i]);

					if (active) {
						render::rect_filled(p.x + DROPDOWN_X_OFFSET + 1, p.y + m_offset + DROPDOWN_BOX_HEIGHT + DROPDOWN_ITEM_Y_OFFSET + item_offset - 1, 2, DROPDOWN_ITEM_HEIGHT, color);
					}
				}
			}
		}
		...
		if (!m_active_items.size())
			render::menu_b.string(p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET,
		...
		// more then one item.
			...
			for (size_t i{}; i < items.size(); ++i) {
				...
				if (render::menu_b.size(tmp.str() + XOR("...")).m_width >= (m_w - DROPDOWN_X_OFFSET - DROPDOWN_ITEM_X_OFFSET - 10)) {
					ss << XOR("...");
					break;
				}
			...
			render::menu_b.string(p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET, p.y + m_offset + 4, { 152, 152, 152, m_parent->m_alpha }, ss.str());
		}
	}
}
