...
float penetration::scale(Player* player, float damage, float armor_ratio, int hitgroup) {
	// credits to LoOsE for reversing this.
	...
	auto IsArmored = [&]()->bool
	{
		...
		{
		case HITGROUP_HEAD:
			return !!player->m_bHasHelmet(); // fuck compiler errors - force-convert it to a bool via (!!)
	...
	{
		float bonusValue = 1.f, armorBonusRatio = .5f, armorRatio = armor_ratio * .5f;
