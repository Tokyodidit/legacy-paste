...
void Shots::OnImpact(IGameEvent* evt) {
	...
	// this is a miss due to wrong angles.
		...
		if (mode == Resolver::Modes::RESOLVE_LBY_UPDATE)
			++data->m_body_index;
	...
	bool ut = g_menu.main.misc.mode.get() == 1 && g_menu.main.aimbot.nospread.get();
	bool CanHit(vec3_t start, vec3_t end, LagRecord * record, int box, bool in_shot, BoneArray * bones);


	//if (g_shots.CanHit(start, end, shot->m_record, shot->m_hitbox, false, nullptr))
		canhit = true;
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (g_menu.main.misc.hitmarker.get()) {
		...
		switch (g_menu.main.misc.hitsounds.get() % 9) {
		case 0: break;
	...
	if (g_menu.main.misc.notifications.get(1)) {
		...
		if (hp == 0 && g_menu.main.misc.killsay.get()) {
			...
			case 0:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say 1'd by a retard XD");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Don't be dumb, get autism.");
				break;

			case 1:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say 1'd by paste");
				else
					g_csgo.m_engine->ExecuteClientCmd("say Get good, get autism.");
				break;

			case 2:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Resolved by math.random");
				else
					g_csgo.m_engine->ExecuteClientCmd("say retard 1d u");
				break;

			case 3:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say Autism");
				else
					g_csgo.m_engine->ExecuteClientCmd("say My bad bro");
				break;

			case 4:
				if (damage > 99)
					g_csgo.m_engine->ExecuteClientCmd("say re$olved");
				else
					g_csgo.m_engine->ExecuteClientCmd("say you should get autism");
				break;
			}
		}
	}
	...
	// if we miss a shot on body update.
	// we can chose to stop shooting at them.
	if (mode == Resolver::Modes::RESOLVE_LBY_UPDATE && data->m_body_index > 0)
		--data->m_body_index;

	else if (mode == Resolver::Modes::RESOLVE_FREESTAND && data->m_freestanding_index > 0)
		--data->m_freestanding_index;

	else if (mode == Resolver::Modes::RESOLVE_BRUTEFORCE && data->m_stand_index2 > 0)
		--data->m_stand_index2;

	else if (mode == Resolver::Modes::RESOLVE_LAST_LBY && data->m_moving_index > 0)
		--data->m_moving_index;
